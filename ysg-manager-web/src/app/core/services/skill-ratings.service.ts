  ...
  implements CrudService
{
  ...
  getSkillRating(id: number): Observable<SkillRating> {
    return this.applicationTournament$.pipe(
      map(
        (applicationTournament) => applicationTournament._links.skillrating.href
      ),
      map((templateUrl) => decodeURIComponent(templateUrl)),
      map((decodedUrl) => decodedUrl.replace(':ratingId', id.toString())),
      flatMap((url) => this.http.get<SkillRating>(url))
    );
  }
  ...
  getSkillRatingTitle(skillRating: SkillRating): string {
    ...
    if (skillRating.player) {
      return `#${skillRating.player.shirtNumber} ${skillRating.player.team.name} (${selectedSkill.name})`;
    } else {
